...
def get_report_field(filters):
	if filters.parent_row_group == "Sale Invoice" or filters.row_group == "Sale Invoice":
		return [
			...
			{"label":"Tax and Charge", "short_label":"Comm.", "fieldname":"tax_and_charge","fieldtype":"Currency","indicator":"Green","precision":None, "align":"right","chart_color":"#FF3D00","sql_expression":"coalesce(b.total_taxes_and_charges,0)"},		
			{"label":"Amount", "short_label":"Amt", "fieldname":"amount","fieldtype":"Currency","indicator":"Red","precision":None, "align":"right","chart_color":"#2E7D32","sql_expression":"sum(a.net_amount) + coalesce(b.total_taxes_and_charges,0)"},
			{"label":"Profit", "short_label":"Prof.", "fieldname":"profit","fieldtype":"Currency","indicator":"Green","precision":None, "align":"right","chart_color":"#FF3D00","sql_expression":"a.net_amount - (a.qty*if(coalesce(c.valuation_rate,0)*a.conversion_factor>a.rate or coalesce(c.valuation_rate,0)<0,(SELECT d.valuation_rate FROM `tabItem` d WHERE d.item_code = a.item_code),coalesce(c.valuation_rate,a.incoming_rate))*a.conversion_factor)"}
		]
	else:
		return [
